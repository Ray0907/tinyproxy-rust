...
fn loopback_is_the_default_and_public_listening_must_be_explicit() {
    ...
    assert_eq!(config.listen_addresses, vec!["127.0.0.1".parse::<std::net::IpAddr>().unwrap()]);
...
fn basic_auth_supports_original_quoted_and_legacy_syntax() {
    let config = Config::parse("BasicAuth alice secret\nBasicAuth \"bob\" \"a b:c#d\" # comment\nBasicAuth charlie:other").unwrap();
...
fn malformed_auth_never_disables_authentication_silently() {
    for text in ["BasicAuth user", "BasicAuth user \"\"", "BasicAuth \"\" pass", "BasicAuth user pass\nBasicAuth user other"] {
...
fn error_messages_include_line_numbers_but_not_credentials() {
    let error = Config::parse("# comment\nBasicAuth user TOP_SECRET extra").err().unwrap();
...
fn unsupported_and_malformed_directives_are_rejected() {
    for text in ["Upstream http://proxy:8080", "User nobody", "Group nobody", "Anonymous Cookie", "FilterURLs maybe", "MaxClients 0", "Timeout 0", "Unknown value", "Port = 8888"] {
...
fn connect_port_replaces_defaults_and_zero_disables_connect() {
    assert_eq!(Config::parse("ConnectPort 8443").unwrap().connect_ports, vec![8443]);
    assert!(Config::parse("ConnectPort 0").unwrap().connect_ports.is_empty());
...
fn acl_preserves_order_and_handles_ipv4_mapped_clients() {
    ...
    assert!(!AccessControl::new(&config).unwrap().is_allowed("127.0.0.1".parse().unwrap()));
}
...
fn invalid_acl_and_missing_filter_fail_at_startup() {
    ...
    let directory = tempfile::tempdir().unwrap();
    let config = Config { filter_file: Some(directory.path().join("absent").to_string_lossy().into()), ..Config::default() };
    assert!(Runtime::new(config).is_err());
}
...
fn domain_filtering_is_enabled_when_filter_urls_is_no() {
    ...
    let config = Config { filter_file: Some(file.path().to_string_lossy().into()), ..Config::default() };
...
fn invalid_regex_is_not_downgraded_to_a_literal() {
    let mut file = NamedTempFile::new().unwrap();
    writeln!(file, "[").unwrap();
    let config = Config { filter_file: Some(file.path().to_string_lossy().into()), filter_extended: true, ..Config::default() };
...
fn whitelist_filter_denies_unmatched_destinations() {
    ...
    let config = Config { filter_file: Some(file.path().to_string_lossy().into()), filter_default_deny: true, ..Config::default() };
    let filter = Filter::new(&config).unwrap();
