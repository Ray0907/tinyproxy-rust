...
impl Config {
    pub fn from_file(path: impl AsRef<Path>) -> Result<Self> {
        ...
        for file in [&mut config.filter_file, &mut config.tls_cert, &mut config.tls_key] {
    ...
    pub fn parse(text: &str) -> Result<Self> {
        ...
        for (index, line) in text.lines().enumerate() {
            ...
            let apply = |config: &mut Self, saw_listen: &mut bool, saw_connect_port: &mut bool| -> Result<()> {
                ensure!(args[0].starts_with(|c: char| c.is_ascii_alphabetic())
                ...
                if key == "basicauth" {
                    ...
                    ensure!(!username.is_empty() && !password.is_empty() && !username.contains(':'), "Invalid BasicAuth credentials");
                    ensure!(!config.basic_auth.iter().any(|auth| auth.username == username), "Duplicate BasicAuth username");
                ...
                match key.as_str() {
                    ...
                    "listen" => {
                        ...
                        if !*saw_listen { config.listen_addresses.clear(); *saw_listen = true; }
                        config.listen_addresses.push(address);
                    }
                    "bind" => config.bind_address = Some(value.parse().context("Invalid Bind address")?),
                    "timeout" => config.timeout = value.parse().context("Invalid Timeout")?,
                    "headertimeout" => config.header_timeout = value.parse().context("Invalid HeaderTimeout")?,
                    "connecttimeout" => config.connect_timeout = value.parse().context("Invalid ConnectTimeout")?,
                    "shutdowntimeout" => config.shutdown_timeout = value.parse().context("Invalid ShutdownTimeout")?,
                    "maxclients" => config.max_clients = value.parse().context("Invalid MaxClients")?,
                    ...
                    "connectport" => {
                        ...
                        if !*saw_connect_port { config.connect_ports.clear(); *saw_connect_port = true; }
                    ...
                    "maxconcurrentstreams" => config.max_concurrent_streams = value.parse().context("Invalid MaxConcurrentStreams")?,
                    "maxinflightrequests" => config.max_inflight_requests = value.parse().context("Invalid MaxInflightRequests")?,
                    "tlscert" => config.tls_cert = Some(value.clone()),
                    "tlskey" => config.tls_key = Some(value.clone()),
                    "tlshandshaketimeout" => config.tls_handshake_timeout = value.parse().context("Invalid TLSHandshakeTimeout")?,
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        ensure!((1..=1_000_000).contains(&self.max_clients), "MaxClients must be in 1..=1000000");
        ensure!((1..=1_000_000).contains(&self.max_inflight_requests), "MaxInflightRequests must be in 1..=1000000");
        ensure!((1..=1024).contains(&self.max_concurrent_streams), "MaxConcurrentStreams must be in 1..=1024");
        ensure!([self.timeout, self.header_timeout, self.connect_timeout, self.shutdown_timeout, self.tls_handshake_timeout]
            .iter().all(|seconds| (1..=86400).contains(seconds)), "Timeouts must be in 1..=86400 seconds");
        ensure!(self.listen_addresses.iter().all(IpAddr::is_loopback)
            || !self.acl_rules.is_empty() || !self.basic_auth.is_empty(),
            "Non-loopback Listen requires explicit Allow/Deny rules or BasicAuth");
        ensure!(!self.via_proxy_name.is_empty() && self.via_proxy_name.bytes().all(|c| c.is_ascii_alphanumeric() || b"._-".contains(&c)),
        ...
        ensure!(self.tls_cert.is_some() == self.tls_key.is_some(), "TLSCert and TLSKey must be configured together");
        ensure!(!self.allow_h2c || self.http2, "AllowH2C requires HTTP2 Yes");
        ensure!(!self.allow_h2c || self.tls_cert.is_none(), "AllowH2C is only for plaintext listeners; TLS uses ALPN");
        Ok(())
    }
}
...
fn tokenize(line: &str) -> Result<Vec<String>> {
    ...
    while let Some(c) = chars.next() {
        if let Some(delimiter) = quote {
            ...
            if active { words.push(std::mem::take(&mut word)); active = false; }
        } else { word.push(c); active = true; }
    }
