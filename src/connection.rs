...
use hyper::header::{HeaderMap, HeaderName, HeaderValue, CONNECTION, COOKIE, HOST, PROXY_AUTHENTICATE, VIA};
...
pub async fn serve(mut stream: TcpStream, address: SocketAddr, runtime: Arc<Runtime>, guard: Arc<ConnectionGuard>) -> Result<()> {
    ...
    let negotiate = async {
        let (h2, io): (bool, BoxIo) = if let Some(acceptor) = &runtime.tls {
            let tls = timeout(Duration::from_secs(runtime.config.tls_handshake_timeout), acceptor.accept(stream)).await??;
            let h2 = tls.get_ref().1.alpn_protocol() == Some(b"h2");
            if h2 {
                let (detected, io) = timeout(Duration::from_secs(runtime.config.header_timeout), protocol::detect(Box::new(tls))).await??;
            ...
            timeout(Duration::from_secs(runtime.config.header_timeout), protocol::detect(Box::new(stream))).await??
        } else {
    ...
    if h2 { runtime.metrics.http2_connections.fetch_add(1, Ordering::Relaxed); }
    ...
    let context = Arc::new(Context {
        runtime: runtime.clone(), guard, activity,
        cancelled: CancellationToken::new(), http1_upgraded: AtomicBool::new(false),
    });
    ...
    let result = if h2 {
        let executor = Executor { tasks: runtime.tasks.clone(), force_shutdown: runtime.force_shutdown.clone() };
        ...
        builder.timer(TokioTimer::new())
            ...
            .max_buf_size(16 * 1024).half_close(true);
    ...
    if h2 || !context.http1_upgraded.load(Ordering::Relaxed) { context.cancelled.cancel(); }
    result
}
...
async fn handle(request: Request<Incoming>, context: Arc<Context>) -> HttpResult<Response<Body>> {
    ...
    if request.extensions().get::<hyper::ext::Protocol>().is_some() {
        return Err((StatusCode::NOT_IMPLEMENTED, "Extended CONNECT is not supported"));
    }
    ...
    let permit = runtime.requests.clone().try_acquire_owned().map_err(|_| {
        runtime.metrics.requests_rejected.fetch_add(1, Ordering::Relaxed);
        (StatusCode::SERVICE_UNAVAILABLE, "Proxy request capacity reached")
    })?;
    let state = Exchange::new(permit, context.guard.clone(), runtime.metrics.clone(),
        context.cancelled.child_token(), Duration::from_secs(runtime.config.timeout));
    ...
    if result.is_ok() { lease.0 = None; }
    result
}

async fn forward(mut request: Request<Incoming>, target: Target, context: Arc<Context>, state: Arc<Exchange>) -> HttpResult<Response<Body>> {
    let runtime = &context.runtime;
    let version = request.version();
    let is_connect = request.method() == Method::CONNECT;
    if !is_connect && runtime.config.stat_host.as_deref() == Some(target.host.as_str()) {
        ...
        let mut response = text_response(StatusCode::OK, runtime.metrics.to_html(), "text/html; charset=utf-8");
        if request.method() == Method::HEAD { *response.body_mut() = empty_body(); }
    ...
    if is_connect {
        ...
        if version != Version::HTTP_2 { context.http1_upgraded.store(true, Ordering::Relaxed); }
        let force = runtime.force_shutdown.clone();
        runtime.tasks.spawn(async move {
            let tunnel = async {
                let upgraded = upgrade.await?;
                let mut client = ActivityIo::new(TokioIo::new(upgraded), state.activity.clone(), None);
...
async fn dial(target: &Target, runtime: &Runtime) -> HttpResult<TcpStream> {
    transport::connect(&target.host, target.port, runtime.config.bind_address, runtime.config.connect_timeout)
        .await.map_err(|error| {
            ...
            } else { (StatusCode::BAD_GATEWAY, "Upstream connection failed") }
        })
}

struct Target { host: String, port: u16, authority: HeaderValue, path: Uri, url: String }
impl Target {
    fn parse<B>(request: &Request<B>) -> std::result::Result<Self, &'static str> {
        ...
        if request.headers().get_all(HOST).iter().count() > 1 { return Err("Multiple Host headers are not allowed"); }
        let authority = if let Some(authority) = uri.authority() {
            authority.clone()
        } else {
            if connect { return Err("CONNECT requires an authority"); }
            request.headers().get(HOST).and_then(|v| v.to_str().ok())
                .ok_or("Host header required")?.parse::<hyper::http::uri::Authority>()
                .map_err(|_| "Invalid Host header")?
        };
        if authority.as_str().contains('@') || authority.host().is_empty() { return Err("Invalid destination authority"); }
        ...
        } else if connect { return Err("CONNECT requires an explicit port"); } else { 80 };
        // H2 :authority and Host cannot name different destinations.
        if request.version() == Version::HTTP_2 {
            if let Some(host) = request.headers().get(HOST) {
                let host = host.to_str().map_err(|_| "Invalid Host header")?
                    .parse::<hyper::http::uri::Authority>().map_err(|_| "Invalid Host header")?;
                if !host.host().eq_ignore_ascii_case(authority.host()) || host.port_u16().unwrap_or(80) != port {
        ...
        if host.is_empty() { return Err("Empty destination host"); }
        let path = uri.path_and_query().map(|p| p.as_str()).unwrap_or("/");
        if !connect && !path.starts_with('/') && !(path == "*" && request.method() == Method::OPTIONS) {
            return Err("Invalid HTTP request target");
        }
        Ok(Self {
            host, port, authority: HeaderValue::from_str(raw).map_err(|_| "Invalid authority header")?,
            path: path.parse().map_err(|_| "Invalid request path")?, url: format!("http://{}{}", raw, path),
        })
    }
}
...
fn validate_h2_headers(headers: &HeaderMap) -> std::result::Result<(), &'static str> {
    for name in ["connection", "proxy-connection", "keep-alive", "transfer-encoding", "upgrade"] {
        if headers.contains_key(name) { return Err("Connection-specific field in HTTP/2"); }
    }
    for value in headers.get_all("te") {
        if !value.as_bytes().eq_ignore_ascii_case(b"trailers") { return Err("HTTP/2 TE must be trailers"); }
    }
    Ok(())
}
...
fn coalesce_cookies(headers: &mut HeaderMap) -> std::result::Result<(), &'static str> {
    if headers.get_all(COOKIE).iter().count() < 2 { return Ok(()); }
    let mut joined = Vec::new();
    for (index, cookie) in headers.get_all(COOKIE).iter().enumerate() {
        if index != 0 { joined.extend_from_slice(b"; "); }
...
pub(crate) fn strip_hop_by_hop(headers: &mut HeaderMap) -> std::result::Result<(), &'static str> {
    if headers.contains_key("content-length") && headers.contains_key("transfer-encoding") { return Err("Ambiguous message framing"); }
    ...
    for value in headers.get_all(CONNECTION) {
        for token in value.to_str().map_err(|_| "Invalid Connection header")?.split(',') {
            ...
            if name == "content-length" || name == "transfer-encoding" { return Err("Connection must not nominate message framing headers"); }
            nominated.push(name);
        }
    }
    for name in nominated { headers.remove(name); }
    for name in ["connection", "proxy-connection", "keep-alive", "proxy-authorization", "proxy-authenticate", "te", "trailer", "transfer-encoding", "upgrade"] {
        headers.remove(name);
    }
    Ok(())
}
...
fn add_via(headers: &mut HeaderMap, runtime: &Runtime, version: Version) {
    if !runtime.config.disable_via_header {
        let protocol = match version { Version::HTTP_2 => "2", Version::HTTP_10 => "1.0", _ => "1.1" };
        let value = HeaderValue::from_str(&format!("{} {}", protocol, runtime.config.via_proxy_name)).expect("startup-validated ViaProxyName");
...
fn error_response(status: StatusCode, message: &'static str, version: Version) -> Response<Body> {
    ...
    if version != Version::HTTP_2 { response.headers_mut().insert(CONNECTION, HeaderValue::from_static("close")); }
