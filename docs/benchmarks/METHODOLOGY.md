...
Resource sampling reads /proc every 100 ms over setup/warmup/measurement. Peak RSS is a sampled resident-process maximum, not virtual address space and not kernel socket memory. CPU percentages refer to one logical CPU; short bursts and process exit can cause sampling error. Direct-to-origin workloads and generator/origin CPU measurements provide checks for test-side bottlenecks, but do not eliminate them.
...
The first attempt used `LogFile /dev/null` for C Tinyproxy and failed at startup. That is a benchmark setup error, not a measured C-proxy failure or a zero-throughput result. The corrected harness uses captured stdout logging and keeps the failed run separate from the completed data set.
